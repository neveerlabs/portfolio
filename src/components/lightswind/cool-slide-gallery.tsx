...
const CoolSlideGallery: React.FC<CoolSlideGalleryProps> = ({
  ...
  // ── Autoplay (pauses when out of view) ──────────────────────────────────
  useEffect(() => {
    if (!autoplay || n < 2 || !isInView) return;
  ...
  }, [autoplay, autoplayDirection, autoplayDelay, n, step, isInView]);
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "relative flex flex-col items-center justify-center select-none overflow-visible will-change-transform",
        className
      )}
      style={{
